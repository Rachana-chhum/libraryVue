        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="book in books" :key="book.id" class="hover:bg-blue-50 transition-colors">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <span
                :class="`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${book.available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
              ...
              <button @click="confirmDelete(book.id)"
                class="text-red-600 hover:text-red-900 disabled:opacity-50 transition-colors" :disabled="isLoading">
                Delete
              </button>
            </td>
          </tr>
        ...
        <div class="p-6">
          ...
          <form @submit.prevent="isEditing ? handleOptimisticUpdate() : handleOptimisticAdd()">
            <div class="mb-4">
              ...
              <input v-model="currentBook.title" type="text" id="title"
            ...
            <div class="mb-4">
              ...
              <input v-model="currentBook.author" type="text" id="author"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border transition-colors"
                required :disabled="isLoading" />
            </div>
            <div class="mb-4">
              <label for="category" class="block text-sm font-medium text-gray-700">Category</label>
              <input v-model="currentBook.category.name" type="text" id="category"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border transition-colors"
                required :disabled="isLoading" />
            </div>
            <div class="mb-4">
              ...
              <div class="mt-1 flex items-center">
                <input v-model="currentBook.available" type="checkbox" id="available"
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all duration-150">
        <div class="p-6">
          ...
          <p class="text-sm text-gray-500 mb-6">Are you sure you want to delete this book? This action cannot be undone.
          </p>
          <div v-if="errorMessage" class="mb-4 text-red-600 text-sm">{{ errorMessage }}</div>
